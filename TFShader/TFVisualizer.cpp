...
{
    // create the VBOs
    FullVertex * vertices = new FullVertex[m_nRows * m_nCols];
    // TODO: for now I'll hardcode FellVertex, but I could use C++ unions to 
    // handle different vertex representations gracefully (BasicVertex, 
    ...
    for (unsigned int c=0; c<m_nCols; c++) {
        for (unsigned int r=0; r<m_nRows; r++) {
            ...
            // vertex array
            vertices[c*m_nRows+r].location[X_POS] = 0;
            vertices[c*m_nRows+r].location[Y_POS] = mat[r][c];
            vertices[c*m_nRows+r].location[Z_POS] = -(GLfloat)r/(m_nRows-1);
            vertices[c*m_nRows+r].color[R_POS] = 1.0;
            vertices[c*m_nRows+r].color[G_POS] = 1.0;
            vertices[c*m_nRows+r].color[B_POS] = 0.0;
            vertices[c*m_nRows+r].color[A_POS] = 1.0;
            vertices[c*m_nRows+r].tex[U_POS] = 0.0;
            vertices[c*m_nRows+r].tex[V_POS] = 0.0;
            vertices[c*m_nRows+r].normal[X_POS] = 0.0;
            vertices[c*m_nRows+r].normal[Y_POS] = 0.0;
            vertices[c*m_nRows+r].normal[Z_POS] = 1.0;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(FullVertex) * (m_nRows*m_nCols), NULL, GL_DYNAMIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(FullVertex) * (m_nRows*m_nCols), vertices); // Actually upload the data
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, (m_nRows*m_nCols) * sizeof(GLuint), indices, GL_STATIC_DRAW);
    
    
    // create and link the sahder program (use the programable OpenGL pipeline)
    m_shaderManager = new ShaderManager( TF_VERTEX_SHADER, TF_FRAGMENT_SHADER );
...
void TFVisualizer::render( float time )
{
    ...
    glUniform3f(displayScale, m_displayScales[X_POS], m_displayScales[Y_POS], m_displayScales[Z_POS]);
    ...
    for (unsigned int c = 0; c < m_nCols; ++c)
    {
        ...
        glDrawElements(GL_LINE_STRIP, m_nRows, GL_UNSIGNED_INT, BUFFER_OFFSET(t0 * m_nRows * sizeof(GLuint)));
    }
    glPopMatrix();
...
{
    if ( length != m_nRows ) {
        std::cerr << "Number of rows missmatch: " << m_nRows << " != " << length << std::endl;
        return;
    }
    
    // For now the data passed in is only the Y value. We need to construct the 
    // corresponding FullVertex object
    FullVertex * vertices = new FullVertex[m_nRows];
    ...
    for (unsigned int r=0; r<m_nRows; r++) {
        ...
        // vertex array
        vertices[r].location[X_POS] = 0;
        vertices[r].location[Y_POS] = column[r];
        vertices[r].location[Z_POS] = -(GLfloat)r/(m_nRows-1);
        vertices[r].color[R_POS] = 1.0;
        vertices[r].color[G_POS] = 1.0;
        vertices[r].color[B_POS] = 0.0;
        vertices[r].color[A_POS] = 1.0;
        vertices[r].tex[U_POS] = 0.0;
        vertices[r].tex[V_POS] = 0.0;
        vertices[r].normal[X_POS] = 0.0;
        vertices[r].normal[Y_POS] = 0.0;
        vertices[r].normal[Z_POS] = 1.0;        
    }
    

    // update the VBO in the graphics card
    glBindBuffer(GL_ARRAY_BUFFER, m_vertexVBO);
    glBufferSubData(GL_ARRAY_BUFFER, m_t0Col*sizeof(FullVertex) * m_nRows, sizeof(FullVertex) * m_nRows, vertices); // Actually upload the data
